...
async function getFrameworkCourses(page, framework) {
  await page.goto(`https://egghead.io/browse/frameworks/${framework}`)
...
async function run() {
  ...
  const fw = process.argv.slice(2)
  const courses = await getFrameworkCourses(page, fw)
